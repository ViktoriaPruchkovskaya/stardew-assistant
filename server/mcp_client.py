...
class MCPClient:
    ...
    async def prune_context(self):
        """Keep in the context only the last user-assistant messages"""
        ...
        old_context_text = "\n".join(
            f"{msg['role'].capitalize()}: {msg['content']}" for msg in old_context
        )
        ...
        self.context = [{"role": "system", "content": f"Summary of earlier conversation:\n{summary_text}"}] + recent_context
